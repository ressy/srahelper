...
# build_sra_submission ----------------------------------------------------


test_that("build_sra_submission makes list of data frames", {
  sample_attrs <- setup_sra_table()
  template <- "MIGS.ba.human-associated.4.0"
  submission <- build_sra_submission(package_name = template,
...
test_that("build_sra_submission works with package display name", {
  sample_attrs <- setup_sra_table()
  template <- "MIGS.ba.human-associated.4.0"
  template_display <- paste("MIGS: cultured bacteria/archaea,",
                            "human-associated; version 4.0")

  submission <- build_sra_submission(package_name = template_display,
                                 biosample_attrs = sample_attrs)
  expect_equal(names(submission), c("biosamples", "metadata"))
  check_biosamples(submission[["biosamples"]], template, sample_attrs)
  check_metadata(submission[["metadata"]])
...
test_that("build_sra_submission handles submission accession", {
  sample_attrs <- setup_sra_table()
  template <- "MIGS.ba.human-associated.4.0"
  sub_acc <- "SUB"
  submission <- build_sra_submission(package_name = template,
...
test_that("build_sra_submission handles col_pairs for both", {
  sample_attrs <- setup_sra_table()
  template <- "MIGS.ba.human-associated.4.0"
...
test_that("build_sra_submission handles constants for both", {
  sample_attrs <- setup_sra_table()
  template <- "MIGS.ba.human-associated.4.0"
...
test_that("build_sra_submission handles unknown template", {
  sample_attrs <- setup_sra_table()
...
test_that("build_sra_submission handles one-to-many for biosamples/metadata", {
...
# validate_sra_submission -------------------------------------------------


test_that("validate_sra_submission handles biosample and metadata attributes", {
  ...
  submission <- build_sra_submission("MIGS.ba.human-associated.4.0",
                                 biosample_attrs = sample_attrs,
                                 metadata_col_pairs = md_col_pairs,
                                 metadata_constants = md_constants)
  submission$biosamples$host <- 1:nrow(submission$biosamples)
  expect_warning(problems <- validate_sra_submission(submission))
...
test_that("write_sra_submission writes both spreadsheets", {
  ...
  dir.create(dp)
  setwd(dp)
  result <- write_sra_submission(submission)
...
test_that("write_sra_submission splits for too many biosamples", {
